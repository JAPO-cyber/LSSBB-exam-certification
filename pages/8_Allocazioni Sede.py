    ...
    if st.session_state.road_data is not None:
        if not st.session_state.calculated:
            ...
            if st.button("Calcola Allocazione"):
                # Esegue il tuo codice originale per l'allocazione
                ...
                # Salva il calcolo nella sessione
                st.session_state.road_data = Road
                st.session_state.calculated = True
                st.success("Calcolo completato!")
        
        # Mostra i risultati se il calcolo è stato eseguito
        if st.session_state.calculated:
            st.write("### Tabella con Risultati")
            st.dataframe(st.session_state.road_data.head(10))

            # Opzione per salvare i risultati
            if st.button("Salva Risultati in Excel"):
                st.session_state.road_data.to_excel("Output_Analisi.xlsx", index=False)
                st.success("Risultati salvati come 'Output_Analisi.xlsx'.")
    else:
